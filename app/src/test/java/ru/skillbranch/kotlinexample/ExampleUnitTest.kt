...
class ExampleUnitTest {
    ...
    fun import_from_csv_success() {
        class ImportDate(
            ...
            fun toText() : String = "$fullName;$email;$password:$salt;$rawPhone"
                .apply { println("Import user: $this") }
        }

        val holder = UserHolder
        val importList = listOf(
            ImportDate(" John Doe ", "JohnDoe@unknow.com", "[B@7591083d", "c6adb4becdc64e92857e1e2a0fd6af84", ""),
            ImportDate("Don Joe", "DonJoe@unknow.com", "[B@7591083d", "", ""),
            ImportDate("Doe John", "", "", "", "+7 (917) 971-11-11")
        )

        holder.importUsers(importList.map { it.toText() }).forEachIndexed { index, user ->
            val importUser = importList[index]
            val resultUser = if (importUser.rawPhone.isNotBlank()) {
                holder.loginUser(importList[index].rawPhone, user.accessCode!!)
            } else {
                holder.loginUser(importList[index].email, importList[index].password)
            }
            val resultSuccess = resultUser != null
            Assert.assertEquals(resultSuccess, true)
        }
    }

    @Test
    fun drop_last_until_success() {
        Assert.assertEquals(
